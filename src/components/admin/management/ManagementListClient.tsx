...
import { ManagementModal, type ManagementRecord } from "./ManagementModal"
import { DeleteConfirmModal } from "./DeleteConfirmModal"
import { toggleManagementStatus } from "@/app/admin/management/actions"

interface ManagementListClientProps {
  initialMembers: ManagementRecord[]
}

export function ManagementListClient({
  initialMembers,
}: ManagementListClientProps) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()

  const [members, setMembers] = useState<ManagementRecord[]>(initialMembers)
  const [prevMembers, setPrevMembers] = useState<ManagementRecord[]>(initialMembers)
  ...
  const [memberToEdit, setMemberToEdit] = useState<ManagementRecord | null>(null)
  const [memberToDelete, setMemberToDelete] = useState<ManagementRecord | null>(null)

  // Toggle loading tracking per member ID
  ...
  const handleToggleStatus = async (member: ManagementRecord) => {
    ...
    const res = await toggleManagementStatus(member.id, member.isActive)
    setTogglingId(null)
  ...
  // Filtered members based on search (uses members state)
  const currentList = members

  const filteredMembers = currentList.filter(
    (m) =>
      m.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      m.position.toLowerCase().includes(searchQuery.toLowerCase())
  )

  const modalKey = memberToEdit
    ? `edit-${memberToEdit.id}`
    : `add-${isAddModalOpen}`
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              O'quv markazi rahbariyati a'zolarini boshqarish
            </p>
          </div>
        </div>
        ...
        >
          ...
          <span>+ Rahbariyat a'zosi qo'shish</span>
        </button>
      </div>
      ...
      {filteredMembers.length === 0 ? (
        <div className="bg-white dark:bg-slate-900 rounded-2xl p-12 text-center border border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 max-w-sm mx-auto">
            {searchQuery
              ? "Qidiruv so'rovingizga mos keladigan a'zo topilmadi."
              : "Hozircha hech qanday rahbariyat a'zosi qo'shilmagan."}
          </p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full text-left text-sm border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/50 text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                    ...
                    <th className="py-3.5 px-4">Qisqacha ma'lumot</th>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredMembers.map((m) => (
                    ...
                    >
                      ...
                      {/* Bio */}
                      <td className="py-3.5 px-4">
                        <div className="text-sm font-medium text-slate-700 dark:text-slate-300 line-clamp-2 max-w-xs">
                          {m.bio || "—"}
                        </div>
                      </td>
              ...
              >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800 text-xs">
                  <span className="text-slate-500 dark:text-slate-400">
                    Tartib: <strong className="text-slate-900 dark:text-white">#{m.sortOrder}</strong>
                  </span>
