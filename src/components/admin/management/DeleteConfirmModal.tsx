...
import { deleteManagement } from "@/app/admin/management/actions"
import type { ManagementRecord } from "./ManagementModal"

interface DeleteConfirmModalProps {
  isOpen: boolean
  member: ManagementRecord | null
  onClose: () => void
  onSuccess: () => void
}
...
export function DeleteConfirmModal({
  ...
  const handleDelete = async () => {
    ...
    const result = await deleteManagement(member.id)
    setLoading(false)
        ...
        <div className="p-6 text-center space-y-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              Rahbariyat a'zosini o'chirishni tasdiqlaysizmi?
            </h3>
