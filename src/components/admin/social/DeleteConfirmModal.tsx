...
import type { SocialRecord } from "./SocialModal"

interface DeleteConfirmModalProps {
  isOpen: boolean
  socialItem: SocialRecord | null
  onClose: () => void
  onSuccess: () => void
}
...
export function DeleteConfirmModal({
  ...
  if (!isOpen || !socialItem) return null

  const handleDelete = async () => {
    setLoading(true)
    setError(null)

    const result = await deleteSocialLink(socialItem.id)
    setLoading(false)
        ...
        <div className="p-6 text-center space-y-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-slate-900 dark:text-white">
              Ijtimoiy tarmoq havolasini o'chirishni tasdiqlaysizmi?
            </h3>
